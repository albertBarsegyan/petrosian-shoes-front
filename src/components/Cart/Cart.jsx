    ...
    <div className="container-cart" style={{ visibility: isVisible ? 'visible' : 'hidden' }}>
      <div className="shopping-cart">
        <ul className="shopping-cart-items">
          {items && items.map((item, i) => <ItemCart
            props={{
              item,
              items,
              setItems,
        ...
        <div className="shopping-cart-header">
          <div className="shopping-cart-total">
            ...
            <span className="main-color-text"><span
              className="currency-old">{currency}</span>{totalPriceString(totalPrice)}</span>
          </div>
        </div>
          ...
          <div className="quantity">
            ...
            <div className="quantity-but">
              <button className="addBtn" onClick={() => {
                setIncrement(true)
                setQuantity(props.items[i].quantity + 1)
              }}></button>

              <button className="minusBtn" style={{ display: quantity === 1 ? 'none' : 'initial' }} onClick={() => {
                if (props.items[i].quantity > 1) {
                  setIncrement(true)
                  setQuantity(props.items[i].quantity - 1)
                }
              }}></button>
