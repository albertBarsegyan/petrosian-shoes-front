...
import FileUpload, { PicturesWall } from '../../components/Admin/FileUpload';
...
export const CreatePage = () => {
    ...
    const submitHandler = async event => {
        if (!((amd || usd || eur || rub) && shortId && avatarImg && name && collectionType !== [] && sizeArr.length)) {
        ...
        if (collectionType !== []) {
            ...
            arr.forEach((el, i) => {
                switch (el) {
                    case 0: arr[i] = 'Spring / Summer';
                        break;
                    case 1: arr[i] = 'Autumn / Winter';
                        break;
                    default: break;
                }
            });
        ...
        <div className='add-item-cont'>
                        ...
                        sizeArr.map((el, i) => {
                            return (
                                <div className="checkbox-container" key={i}>
                                    ...
                                    <label htmlFor={i}><span className="checkmark"></span></label>
                ...
                <div style={{ width: '100%' }}>
                    {
                        detailList.map((el, i) => {
                            return (<input
                                ...
                                type="text"
                                key={i}
                                onChange={e => { changeDetailListComponent(e, i) }}
                            />)
                        })
                    }
                </div>
                <button onClick={() => { setDetailList(detailList.concat('')) }}>+</button>
            </div>
            <div className="input-field">
            ...
            <div className='row'>
                <div className="input-field">
                    ...
                    <FileUpload setImgs={[setAvatarImg, setHoverImg]} id={shortId} type='avatar' />
                </div>
                <div className="input-field">
                    <label htmlFor='hover'>On hover</label>
                    <FileUpload setImgs={[setAvatarImg, setHoverImg]} id={shortId} type='hover' />
                </div>
            </div>
