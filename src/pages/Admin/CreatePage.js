...
export const CreatePage = () => {
  ...
  function generateSizeArr () {
    let arr = []
    const shoesSizeDiff = type === 'Woman' ? ShouesSizeByGender.Woman : ShouesSizeByGender.Man
    ...
    for (let i = Number(sizeRange1); i <= Number(sizeRange2); i += 1) {
      arr.push(i + ' EU / ' + Number(i - shoesSizeDiff) + ' US')
    }
    if (arr !== []) {
      setSizeArrBool(true)
      setSizeArr(arr)
