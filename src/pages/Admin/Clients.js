...
export const Clients = () => {
  ...
  return (
    <div className="array-purchase-wrap">
      ...
      >
        <TabList>
          <Tab disabled={!(payed && payed.length)}>{payed.length ? <span
            className="red">Payed ( {payed.length} )</span> : 'Payed'}</Tab>
          <Tab disabled={!(shipments && shipments.length)}>Shipments ( {shipments.length ? <span
            className="red">{shipments.length}</span> : 0} )</Tab>
          <Tab disabled={!(notPayed && notPayed.length)}>Not Payed ( {notPayed.length ? <span
            className="red">{notPayed.length}</span> : 0} )</Tab>
