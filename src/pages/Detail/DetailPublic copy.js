...
export const DetailPublic = () => {
  ...
  item.collectionImg.forEach((el, i) => {
    return (arr.push(process.env.PUBLIC_URL + `/upload/${item.shortId}/thumb.${el}`))
  })
  ...
  return (
    <div className='detail-item'>
      <div className='detail-gallery'>
        ...
        {viewerIsOpen ? (
          <ModalGateway>
            <Modal onClose={closeLightbox} className={isZoomed ? 'zoomed' : ''} onClick={() => setIsZoomed(true)}>
              <Carousel
      ...
      <div className='detail-info-cont'>
        <div className='detail-info'>
          ...
          <div className='prices-cont-det'>
            {item.sale &&
              <p className='item-price new-p'><span className='currency'>{currency}</span>{item.newPrices[priceI]}</p>}
            <p className={`item-price ${item.sale ? 'old-p' : ''}`}><span
              className='currency-old'>{currency}</span>{item.prices[priceI]}</p>
          </div>
